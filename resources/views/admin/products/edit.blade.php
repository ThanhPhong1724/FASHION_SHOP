            ...
            <div class="p-6 text-gray-900">
                ...
                <form action="{{ route('admin.products.update', $product) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-8">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <input type="text" name="slug" id="slug" value="{{ old('slug', $product->slug) }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" placeholder="Sẽ tự động tạo nếu để trống">
                            </div>
                            ...
                            <div>
                                ...
                                <input type="text" name="sku" id="sku" value="{{ old('sku', $product->sku) }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                            </div>
                    ...
                    <div class="mb-8">
                        ...
                        <div class="space-y-4">
                            <div>
                                ...
                                <textarea name="short_description" id="short_description" rows="3" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">{{ old('short_description', $product->short_description) }}</textarea>
                            </div>
                    ...
                    <div class="mb-8">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input type="number" name="base_price" id="base_price" value="{{ old('base_price', $product->base_price) }}" min="0" step="1000" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" required>
                            </div>

                            <div>
                                <label for="sale_price" class="block text-sm font-medium text-gray-700">Giá khuyến mãi</label>
                                <input type="number" name="sale_price" id="sale_price" value="{{ old('sale_price', $product->sale_price) }}" min="0" step="1000" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="mb-8">
                            ...
                            <div class="mb-4">
                                ...
                                <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                                    @foreach($product->getMedia('images') as $image)
                                        <div class="relative">
                                            <img src="{{ $image->getUrl('thumb') }}" alt="Product image" class="w-full h-24 object-cover rounded">
                                        </div>
                                    @endforeach
                                </div>
                            </div>
                        @endif
                        <div>
                            <label for="images" class="block text-sm font-medium text-gray-700">Upload hình ảnh mới</label>
                            ...
                            <p class="text-sm text-gray-500 mt-1">Có thể chọn nhiều ảnh cùng lúc (sẽ thêm vào ảnh hiện tại)</p>
                        </div>
                    </div>
                            ...
                            @foreach($product->variants as $index => $variant)
                                <div class="variant-item border border-gray-200 rounded-lg p-4 mb-4">
                                    <div class="flex justify-between items-center mb-2">
                                        <h4 class="text-sm font-medium text-gray-700">Biến thể {{ $index + 1 }}</h4>
                                        <button type="button" class="remove-variant text-red-600 hover:text-red-800 text-sm">Xóa</button>
                                    </div>
                                    <div class="grid grid-cols-1 md:grid-cols-5 gap-4">
                                        <div>
                                            <label class="block text-sm font-medium text-gray-700">Size</label>
                                            <input type="text" name="variants[{{ $index }}][size]" value="{{ $variant->size }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" placeholder="S, M, L, XL">
                                        </div>
                                        <div>
                                            <label class="block text-sm font-medium text-gray-700">Màu sắc</label>
                                            <input type="text" name="variants[{{ $index }}][color]" value="{{ $variant->color }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" placeholder="Đen, Trắng, Xanh">
                                        </div>
                                        <div>
                                            <label class="block text-sm font-medium text-gray-700">SKU</label>
                                            <input type="text" name="variants[{{ $index }}][sku]" value="{{ $variant->sku }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                                        </div>
                                        <div>
                                            <label class="block text-sm font-medium text-gray-700">Tồn kho</label>
                                            <input type="number" name="variants[{{ $index }}][stock]" min="0" value="{{ $variant->stock }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                                        </div>
                                        <div>
                                            <label class="block text-sm font-medium text-gray-700">Điều chỉnh giá</label>
                                            <input type="number" name="variants[{{ $index }}][price_delta]" step="1000" value="{{ $variant->price_delta }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                                        </div>
                                    </div>
                    ...
                    <div class="mb-8">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input type="text" name="meta_title" id="meta_title" value="{{ old('meta_title', $product->meta_title) }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                            </div>

                            <div>
                                <label for="meta_description" class="block text-sm font-medium text-gray-700">SEO Description</label>
                                <input type="text" name="meta_description" id="meta_description" value="{{ old('meta_description', $product->meta_description) }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                            </div>
                        </div>
                    </div>
